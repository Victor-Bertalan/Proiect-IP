...
private:
	void setupRoutes() {
        ...
        Routes::Get(router, "/auth", Routes::bind(&Endpoint::doAuth, this));
        Routes::Post(router, "/settings/:settingName/:value", Routes::bind(&Endpoint::setSettings, this));
        // initializeaza nivelul bateriei 100%
        Routes::Post(router, "/init/:value", Routes::bind(&Endpoint::initBattery, this));
        Routes::Get(router, "/baterie", Routes::bind(&Endpoint::getBattery, this));
        Routes::Get(router, "/settings/:settingName", Routes::bind(&Endpoint::getSettings, this));
        Routes::Post(router, "/goToCharge", Routes::bind(&Endpoint::goToCharge, this));
        // stop-resume-start
        Routes::Post(router, "/:stateName", Routes::bind(&Endpoint::setState, this));
        Routes::Get(router, "/state", Routes::bind(&Endpoint::getState, this));
        Routes::Get(router, "/buildRoad", Routes::bind(&Endpoint::getRoad, this));
